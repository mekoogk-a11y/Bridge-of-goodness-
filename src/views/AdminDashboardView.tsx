...
export const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({ onNavigate, lang }) => {
  ...
  const [activeTab, setActiveTab] = useState<
    'dashboard' | 'volunteers' | 'partnerships' | 'articles' | 'projects' | 'map' | 'settings'
  >('dashboard');
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="p-4 rounded-2xl bg-amber-50/80 border border-amber-300">
                  <span className="text-xs text-amber-800 font-bold block">
                    {isEn ? 'Proposed Projects' : 'المشاريع المقترحة'}
                  </span>
              ...
              <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 text-xs flex items-start gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="leading-relaxed">
                    {isEn
                      ? 'All volunteer registrations and partnership submissions are stored securely in local browser storage for concept verification.'
                      : 'يتم تسجيل جميع طلبات التطوع والشراكة بنظام تخزين محلي آمن، مع التأكيد على أن المبادرة في طور التأسيس ولا تجمع أي تبرعات مالية.'}
                  </p>
                </div>
              </div>
            </div>
